...
#
cpi = pd.read_excel(
    'data/IMF - CPI - 1989to2022 - 20230210.xlsx',
    skiprows=6)

# 
cpi = cpi[cpi.COU.notna()]

#
cpi = pd.melt(
    cpi,
    id_vars= ['COU', 'Country'],
    value_vars= [str(x) for x in range(1989,2023)],
    var_name= 'Year',
    value_name= 'cpi')

#
cpi.cpi = cpi.cpi.replace('...', np.NaN)
df_aux = pd.read_excel('data/ARG_cpi.xlsx')

cpi = pd.concat([cpi, df_aux])

#
cpi.COU = cpi.COU.astype(str)
...
for col in ['4000', '4100', '4000_pctchg', '4100_pctchg']:
    ...
    df_aux = df_aux.dropna(subset=[col, 'cpi_pctchg'])
    ...
    #
    asd = df_aux.groupby('COU')['cpi_pctchg'].agg([iqr, q25, q75])
    ...
    #
    df_aux = df_aux[(
        (df_aux['cpi_pctchg'] < df_aux['ubound'])
        &
        (df_aux['cpi_pctchg'] > df_aux['lbound'])
        )]

    # df_aux.to_excel(f'{col}.xlsx')
    #
    ...
    #  
    pais_lista = ['ARG', 'COD' ,'TUR' ,'GHA' ,'MWI' ,'NGA' ,'CRI' ,'EGY' ,'MDG' ,'PAK' ,'KEN' ,'JAM' ,'URY' ,'MNG' ,'SLE' ,'BWA' ,'HND' ,'KAZ' ,'SLB' ,'RWA' ,'SWZ' ,'PRY' ,'LAO' ,'BRA' ,'KGZ' ,'BTN' ,'IDN' ,'GTM' ,'COL' ,'BGD' ,'LSO', 'NIC', 'BGR', 'DOM', 'ZAF', 'UGA', 'GEO', 'HUN', 'PNG', 'NAM', 'TTO', 'MEX', 'ECU']
...
#    
vbles = ['const', 'cpi_pctchg']
table = pd.DataFrame([], index=['const', 'cpi_pctchg', 'n'])
